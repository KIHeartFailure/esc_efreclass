...
tab1func <- function(data, stratavar, tabname) {
  ...
  footnote(
    default_kable(tab,
      font_size = 6,
      caption = paste0("Baseline characteristics by EF Echo-Doppler ", tabname),
      longtable = TRUE,
      escape = FALSE
    ), # %>%
    # landscape(),
...
tab1func(data = pdata %>% filter(num_Ef_cat2 != "missing"), stratavar = "num_Ef_cat2", tabname = "(groups <41, 41-49, >=50)")
```
...
tab1func(data = pdata %>% filter(num_Ef_cat != "missing"), stratavar = "num_Ef_cat", tabname = "(groups <40, 40-49, >=50)")
```
...
tab1func(data = pdata, stratavar = "num_Ef_missing", tabname = "missing")
```

\clearpage 

```{r tab1ef05, cache=cacheon, dependson="tab1"}
tab1func(data = pdata %>% filter(num_Ef_05cat != "missing"), stratavar = "num_Ef_05cat", tabname = "0,5 vs other")
```

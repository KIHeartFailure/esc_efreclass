...
kmfunc <- function(efvars, time, event, eventname, yposplus = rep(0, 6), lrxy = c(30 * .2, 0.16)) {
  ...
  hrp <- c(
    ...
    paste0(
      ...
      "-", fn(mod$conf.int[, 4], dig = 1), "), p",
      fn(mod$coef[, 5], dig = 2, p = TRUE, pequalsign = T)
    ),
    paste0(
      ...
      "-", fn(mod2$conf.int[2:3, 4], dig = 1), "), p",
      fn(mod2$coef[2:3, 5], dig = 2, p = TRUE, pequalsign = T)
    )
  )
...
kmfunc <- function(efvars, time, event, eventname, yposplus = rep(0, 3), lrxy = c(30 * .2, 0.16)) {
  fit <- survfit(formula(paste0("Surv(", time, ",", event, "== 1) ~ ", efvars)),
    data = survdata
  )
  ## cox
  mod <- summary(coxph(formula(paste0("Surv(", time, ",", event, "== 1) ~ relevel(", efvars, ", ref = 'non-05%')")),
    data = survdata
  ))
  ...
  hrp <- c(
    ...
    "non-05%: reference",
    paste0(
      gsub(paste0("relevel(", efvars[1], ", ref = \"non-05%\")"), "", rownames(mod$conf.int), fixed = TRUE), ": ",
      fn(mod$conf.int[, 1], dig = 1),
      " (", fn(mod$conf.int[, 3], dig = 1),
      "-", fn(mod$conf.int[, 4], dig = 1), "), p",
      fn(mod$coef[, 5], dig = 2, p = TRUE, pequalsign = T)

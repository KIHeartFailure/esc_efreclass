...
kmfunc <- function(time, event, eventname, ymax = 1) {
  ...
  mtext(levs2$levsprint[6], side = 1, line = 9.7, at = -80, adj = 0, cex = cexmy)
  ...
  axis(1,
    at = seqtime, labels = nriskboth$nrisk[nriskboth$levs2 == levs2$levs[6]],
    line = 8.7, tick = FALSE, cex.axis = cexmy
  )
}
```

```{r km, cache = cacheon, dependson="kmfunc", fig.cap = "Cumulative incidence curves", fig.show='hold', out.width="50%", fig.subcap=c('All-cause mortality', 'CVD', 'CVD/First HFH', 'First HFH'), fig.ncol=2}
...
```{r kmdigfunc, cache=cacheon}
kmdigfunc <- function(time, event, eventname, ymax = 1) {
  fits <- survfit(formula(paste0("Surv(", time, ",", event, "== 1) ~ num_Ef_05cat")),
    data = pdata %>% filter(survpop)
  )
  ## cox
  smod <- summary(coxph(formula(paste0("Surv(", time, ",", event, "== 1) ~ relevel(num_Ef_05cat, ref = 'Not 05%')")),
    data = pdata %>% filter(survpop)
  ))
  ...
  levs <- levels(pdata$num_Ef_05cat)
  levsprint <- str_replace_all(levs, "%", " digit")

  legend(
    x = 0.5, y = ymax + 0.05, bty = "n", c("", levsprint),
    col = c("white", global_cols[c(1, 3, 5)]), cex = cexmy, lwd = 2, lty = c(1, 1, 1, 1),
  ...
  mtext("No. at risk", side = 1, line = 3.7, at = -80, adj = 0, cex = cexmy, font = 2)
  mtext(levsprint[1], side = 1, line = 4.7, at = -80, adj = 0, cex = cexmy)
  mtext(levsprint[2], side = 1, line = 5.7, at = -80, adj = 0, cex = cexmy)
  mtext(levsprint[3], side = 1, line = 6.7, at = -80, adj = 0, cex = cexmy)

  nrisk <- summary(fits, seqtime, extend = T)

  axis(1,
    at = seqtime, labels = nrisk$n.risk[nrisk$strata == paste0("num_Ef_05cat=", levs[1])],
    line = 3.7, tick = FALSE, cex.axis = cexmy
  )
  axis(1,
    at = seqtime, labels = nrisk$n.risk[nrisk$strata == paste0("num_Ef_05cat=", levs[2])],
    line = 4.7, tick = FALSE, cex.axis = cexmy
  )
  axis(1,
    at = seqtime, labels = nrisk$n.risk[nrisk$strata == paste0("num_Ef_05cat=", levs[3])],
    line = 5.7, tick = FALSE, cex.axis = cexmy
  )
}
```

```{r kmdig, cache = cacheon, dependson="kmdigfunc", fig.cap = "Cumulative incidence curves", fig.show='hold', out.width="50%", fig.subcap=c('All-cause mortality', 'CVD', 'CVD/First HFH', 'First HFH'), fig.ncol=2}

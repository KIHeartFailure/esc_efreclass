...
tabdigmissfunc <- function(data, tabvars = tabdigvars, tabtitle = "") {
  ...
  default_kable(tabdata,
    font_size = 7,
    caption = sanitize_text(paste0("% Missing data for selected variables by 5% digits EF Echo-Doppler", tabtitle)),
    longtable = TRUE,
    escape = FALSE
  )
}
...
tabdigmissfunc(data <- pdata %>% filter(num_Ef_05cat != "missing"))
```
...
tabdigmissfunc(data <- pdata %>% filter(num_Ef_05cat != "missing", num_dmPtype == "Hospital"),
  tabvars = tabhospvars,
